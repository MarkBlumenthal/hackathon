...
#     def __init__(self, window):
#         self.window = window
#         self.window.title("Welcome to Fight Anti-Semitism")
...
#         # Welcome screen widgets
#         self.welcome_label = tk.Label(window, text="Welcome To Fight Anti-Semitism!")
#         self.welcome_label.pack()

#         self.question_label = tk.Label(window, text="Let's Change people's views in this world?")
#         self.question_label.pack()
...
#         # Placeholder for the story loaded from JSON
#         self.story = []
        
#         # Placeholder for action methods mapping
#         self.action_methods = {
#             "educate_person": self.educate_person,
#             "ignore_person": self.ignore_person,
...
#             # Load the story and start the storyline after the player enters their name
#             self.load_story("storyline.json")
#             self.display_story_part(0)
...
#     # New Methods for Storyline Integration
...
#     def display_story_part(self, part_index=0):
#         """Displays a part of the story and the choices available to the player."""
#         if part_index < len(self.story):
#             story_part = self.story[part_index]
#             self.output_text.insert(tk.END, story_part["text"] + "\n\n")
            
#             # Clear existing game action buttons
#             for widget in self.game_frame.winfo_children():
#                 widget.destroy()
            
#             # Create buttons for each choice in the story part
#             for choice in story_part["choices"]:
#                 button = tk.Button(self.game_frame, text=choice["text"],
#                                    command=lambda action=choice["action"], index=part_index: self.handle_choice(action, index+1))
#                 button.pack(side=tk.LEFT, padx=5, pady=5)
#         else:
#             self.output_text.insert(tk.END, "The story has ended. Thank you for playing!\n")

#     def handle_choice(self, action, part_index):
#         """Handles the player's choice by executing the associated function and displaying the next part of the story."""
#         if action in self.action_methods:
#             self.action_methods[action]()  # Execute the action method
#         self.display_story_part(part_index)  # Display the next part of the story

#     # Example Action Methods

#     def educate_person(self):
#         self.output_text.insert(tk.END, "You take a moment to educate the person. They seem to understand your point.\n")

#     def ignore_person(self):
#         self.output_text.insert(tk.END, "You decide it's best to not engage and walk away.\n")

#     def report_person(self):
#         self.output_text.insert(tk.END, "You report the person to a community leader who promises to handle the situation.\n")

#     # Add other action methods as needed based on your story choices
